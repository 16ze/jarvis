            ...
            try:
                ...
                for _turn in range(6):
                    ...
                    if "error" in data:
                        ...
                        print(
                            f"[TextAgent] Fallback {model} indisponible (code={err_code}) — essai suivant"
                        )
                    ...
                    if not tool_calls:
                        ...
                        if content:
                            print(
                                f"[TextAgent] Fallback LLM OK via {model} (tour {_turn + 1})"
                            )
                    ...
                    for tc in tool_calls:
                        ...
                        try:
                            ...
                            tool_args = {}
                        print(
                            f"[TextAgent] Fallback tool: {tool_name} args={tool_args}"
                        )
                        try:
...
_CORE_TOOL_DEFS = [
    ...
    {
        "name": "read_emails",
        "description": "Lit les emails récents de Gmail. Par défaut retourne les derniers emails reçus.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "query": {
                    "type": "STRING",
                    "description": "Requête Gmail optionnelle. Laisser vide pour les derniers emails. Ex: 'is:unread' pour non lus, 'from:boss@example.com' pour filtrer par expéditeur.",
                },
                "max_results": {
                    "type": "INTEGER",
                    "description": "Nombre d'emails (défaut 5)",
                },
            },
        },
    },
    ...
    {
        "name": "get_email_body",
        "description": "Récupère le corps complet d'un email par son ID.",
        "parameters": {
            "type": "OBJECT",
    ...
    {
        "name": "find_event",
        "description": "Cherche un événement dans Google Calendar.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "query": {"type": "STRING"},
    ...
    {
        "name": "search_memory",
        "description": "Recherche dans la mémoire persistante d'Ada.",
        "parameters": {
            "type": "OBJECT",
    ...
    {
        "name": "remember",
        "description": "Mémorise une information de façon persistante.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "content": {"type": "STRING"},
                "category": {
                    "type": "STRING",
                    "description": "facts, preferences, projects, entity",
                },
    ...
    },
    {
        ...
        "description": "Recherche dans les documents uploadés.",
        "parameters": {
            "type": "OBJECT",
            "properties": {"query": {"type": "STRING"}},
            "required": ["query"],
    ...
    {
        "name": "run_terminal",
        "description": "Exécute une commande shell sur le Mac de Monsieur.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "command": {"type": "STRING"},
    ...
    # Twilio
    {
        ...
        "parameters": {
            ...
            "properties": {
                "to": {
                    "type": "STRING",
                    "description": "Numéro de téléphone du destinataire (format E.164, ex: +33612345678)",
                },
...
class TextAgent:
    ...
    async def _execute_tool(self, name: str, args: dict) -> str:
        ...
        if name == "read_emails" and self._google:
            ...
            return await asyncio.to_thread(
                self._google.get_email_body, args["message_id"]
            )
        elif name == "list_events" and self._google:
            return await asyncio.to_thread(
                self._google.list_events, max_results=args.get("max_results", 10)
            )
        ...
        # ── SLACK ────────────────────────────────────────────────────────────
            ...
            return await asyncio.to_thread(
                self._slack.read_channel, args["channel_id"], args.get("limit", 20)
            )
        elif name == "slack_send_message" and self._slack:
            return await asyncio.to_thread(
                self._slack.send_message, args["channel_id"], args["text"]
            )
        elif name == "slack_search_messages" and self._slack:
            return await asyncio.to_thread(
                self._slack.search_messages, args["query"], args.get("count", 10)
            )

        # ── TELEGRAM ────────────────────────────────────────────────────────
        elif name == "telegram_send_message" and self._telegram:
            return await asyncio.to_thread(
                self._telegram.send_message, args["text"], args.get("chat_id")
            )
            ...
            return await asyncio.to_thread(
                self._telegram.get_updates, args.get("limit", 10)
            )

        # ── WHATSAPP ─────────────────────────────────────────────────────────
        elif name == "whatsapp_send_message" and self._whatsapp:
            return await asyncio.to_thread(
                self._whatsapp.send_message, args["number"], args["text"]
            )
        elif name == "whatsapp_send_media" and self._whatsapp:
            return await asyncio.to_thread(
                self._whatsapp.send_media,
                args["number"],
                args["media_url"],
                args.get("caption", ""),
            )
            ...
            return await asyncio.to_thread(
                self._whatsapp.get_recent_messages,
                args["number"],
                args.get("limit", 20),
            )
        ...
        # ── NOTION ───────────────────────────────────────────────────────────
            ...
            return await asyncio.to_thread(
                self._notion.search, args["query"], args.get("limit", 10)
            )
            ...
            return await asyncio.to_thread(
                self._notion.query_database,
                args["database_id"],
                args.get("filter_json", ""),
            )
        elif name == "notion_append_page" and self._notion:
            return await asyncio.to_thread(
                self._notion.append_to_page, args["page_id"], args["content"]
            )

        # ── GOOGLE DRIVE / SHEETS / DOCS ─────────────────────────────────────
        elif name == "drive_list_files" and self._drive:
            return await asyncio.to_thread(
                self._drive.list_files, args.get("query", ""), args.get("limit", 10)
            )
        ...
        # ── LINEAR ───────────────────────────────────────────────────────────
            ...
            return await asyncio.to_thread(
                self._linear.list_projects, args.get("team_id", "")
            )
        ...
        # ── STRIPE ───────────────────────────────────────────────────────────
            ...
            return await asyncio.to_thread(
                self._stripe.send_invoice, args["invoice_id"]
            )
        ...
        # ── SUPABASE ─────────────────────────────────────────────────────────
            ...
            return await asyncio.to_thread(
                self._supabase.insert_row, args["table"], args["data_json"]
            )
            ...
            return await asyncio.to_thread(
                self._supabase.delete_row, args["table"], args["filters_json"]
            )
        ...
        # ── VERCEL ───────────────────────────────────────────────────────────
            ...
            return await asyncio.to_thread(
                self._vercel.list_projects, args.get("limit", 20)
            )
            ...
            return await asyncio.to_thread(
                self._vercel.list_deployments,
                args.get("project_id", ""),
                args.get("limit", 10),
            )
        elif name == "vercel_get_deployment" and self._vercel:
            return await asyncio.to_thread(
                self._vercel.get_deployment, args["deployment_id"]
            )
        elif name == "vercel_get_logs" and self._vercel:
            return await asyncio.to_thread(
                self._vercel.get_deployment_logs, args["deployment_id"]
            )

        # ── GITHUB ───────────────────────────────────────────────────────────
        elif name == "github_list_repos" and self._github:
            return await asyncio.to_thread(
                self._github.list_repos, args.get("limit", 20)
            )
        elif name == "github_get_repo" and self._github:
            return await asyncio.to_thread(
                self._github.get_repo_info, args.get("repo", "")
            )
            ...
            return await asyncio.to_thread(
                self._github.search_code, args["query"], args.get("repo", "")
            )

        # ── DOCKER ───────────────────────────────────────────────────────────
        elif name == "docker_list_containers" and self._docker:
            return await asyncio.to_thread(
                self._docker.list_containers, args.get("all", False)
            )
        elif name == "docker_get_logs" and self._docker:
            return await asyncio.to_thread(
                self._docker.get_container_logs, args["container"], args.get("tail", 50)
            )
        elif name == "docker_start" and self._docker:
            return await asyncio.to_thread(
                self._docker.start_container, args["container"]
            )
        elif name == "docker_stop" and self._docker:
            return await asyncio.to_thread(
                self._docker.stop_container, args["container"]
            )
        elif name == "docker_restart" and self._docker:
            return await asyncio.to_thread(
                self._docker.restart_container, args["container"]
            )
            ...
            return await asyncio.to_thread(
                self._docker.container_stats, args["container"]
            )
        ...
        # ── SPOTIFY ──────────────────────────────────────────────────────────
            ...
            return await asyncio.to_thread(
                self._spotify.play, args.get("uri", ""), args.get("device_id", "")
            )
            ...
            return await asyncio.to_thread(
                self._spotify.set_volume, args["volume_percent"]
            )
        ...
        # ── YOUTUBE ──────────────────────────────────────────────────────────
            ...
            return await asyncio.to_thread(
                self._yt.search_videos, args["query"], args.get("limit", 5)
            )

        # ── WIKIPEDIA ────────────────────────────────────────────────────────
        elif name == "wikipedia_search" and self._wiki:
            return await asyncio.to_thread(
                self._wiki.search, args["query"], args.get("limit", 5)
            )
        ...
        # ── SELF-CORRECTION ──────────────────────────────────────────────────
            ...
            return self._self_correction.git_commit(
            ...
            if not path.startswith("/"):
                ...
                path = str(_Path(JARVIS_ROOT) / path)
            return self._self_correction.correct_file(
        ...
        # ── CAMÉRA TUYA PTZ ───────────────────────────────────────────────────
            ...
            return await self._tuya_camera.ptz_move(
            ...
            async def _on_motion_bridge(_snap):
                ...
                await _send_text(
                    "telegram", TELEGRAM_CHAT_ID, "⚠️ Mouvement détecté par la caméra !"
                )
                if _snap and _tg:
                    ...
                    await asyncio.to_thread(
                        _tg.send_photo, f"file://{_tmp.name}", "📸 Mouvement détecté"
                    )

            _bg_task(
                self._tuya_camera.start_motion_watch(
                    _on_motion_bridge, with_snapshot=_with_snap
                ),
        ...
        # ── DOMOTIQUE (Tuya) ──────────────────────────────────────────────────
            ...
            for ip, d in self._tuya.devices.items():
                dev_type = (
                    "ampoule" if d.is_bulb else "prise" if d.is_plug else "inconnu"
                )
            ...
            if action == "turn_on":
                ...
                result = (
                    f"'{target}' allumé." if success else f"Échec allumage '{target}'.'"
                )
                ...
                result = (
                    f"'{target}' éteint."
                    if success
                    else f"Échec extinction '{target}'.'"
                )
            elif action == "set":
                success = True
        ...
        elif name == "describe_screen" and _ada_loop:
            return await _ada_loop.screen_watcher.describe()

        # ── RECHERCHE (compléments) ───────────────────────────────────────────
        elif name == "wikipedia_article" and self._wiki:
            return await asyncio.to_thread(
                self._wiki.get_article, args.get("title", ""), args.get("lang", "fr")
            )
        elif name == "arxiv_paper" and self._arxiv:
            return await asyncio.to_thread(
                self._arxiv.get_paper, args.get("arxiv_id", "")
            )
        elif name == "youtube_video_info" and self._yt:
            return await asyncio.to_thread(
                self._yt.get_video_info, args.get("video", "")
            )
        elif name == "youtube_transcript" and self._yt:
            return await asyncio.to_thread(
                self._yt.get_transcript, args.get("video", "")
            )

        # ── SPOTIFY (compléments) ─────────────────────────────────────────────
        elif name == "spotify_playlists" and self._spotify:
            return await asyncio.to_thread(
                self._spotify.get_playlists, args.get("limit", 20)
            )
        ...
        # ── CRÉATION (compléments) ────────────────────────────────────────────
            ...
            return await asyncio.to_thread(
                self._replicate.generate_image,
                args.get("prompt", ""),
            ...
            return await asyncio.to_thread(
                self._replicate.run_model,
                args.get("model_version", ""),
                args.get("input_json", ""),
            )
        ...
        if env_var and not os.getenv(env_var):
            return (
                f"CONFIGURATION MANQUANTE — '{name}' nécessite la variable {env_var} "
    ...
    async def run(self, text: str) -> str:
        ...
        for _ in range(8):  # max 8 tours pour éviter les boucles infinies
            ...
            for _retry in range(3):
                try:
                    ...
                    if _is_quota_error(_gemini_err):
                        print(
                            f"[TextAgent] Quota Gemini épuisé — basculement OpenRouter ({_gemini_err})"
                        )
                        return await _run_fallback_llm(system, original_text)
                    raise
            ...
            tool_result_parts = await asyncio.gather(
                *[_exec(fc) for fc in function_calls]
            )
    ...
    if GEMINI_API_KEY:
        try:
            ...
            if _is_quota_error(e):
                print(
                    f"[ExternalBridge] TTS Gemini quota épuisé — basculement edge-tts"
                )
            else:
...
async def _text_to_ogg_edge(text: str) -> bytes | None:
    ...
    try:
        ...
        if not shutil.which("ffmpeg"):
            warnings.warn(
                "[ExternalBridge] edge-tts: ffmpeg absent — impossible de convertir en OGG"
            )
            return None
...
async def _send_text(
    source: Literal["telegram", "whatsapp"], sender: str, text: str
) -> None:
    if source == "telegram":
...
async def _send_voice(
    source: Literal["telegram", "whatsapp"], sender: str, ogg_bytes: bytes
) -> None:
    if source == "telegram":
...
async def _download_telegram_file(file_id: str) -> bytes | None:
    ...
    try:
        async with httpx.AsyncClient(timeout=20) as c:
            ...
            if not data.get("ok"):
                warnings.warn(
                    f"[ExternalBridge] getFile erreur: {data.get('description')}"
                )
                return None
            ...
            contents=[
                types.Content(
                    parts=[
                        ...
                        types.Part(
                            text="Transcris exactement ce message vocal en français. Retourne uniquement la transcription, sans commentaire."
                        ),
                    ]
                ),
            ],
...
async def _telegram_polling_loop() -> None:
    if not TELEGRAM_BOT_TOKEN:
        warnings.warn(
            "[ExternalBridge] TELEGRAM_BOT_TOKEN manquant — polling désactivé"
        )
        return
            ...
            try:
                ...
                for update in data.get("result", []):
                    ...
                    if allowed_chat and chat_id != allowed_chat:
                        print(
                            f"[ExternalBridge] Message ignoré (chat_id={chat_id} ≠ {allowed_chat})"
                        )
                        continue
                    ...
                    if not text:
                        ...
                        if voice_obj:
                            ...
                            print(
                                f"[ExternalBridge] Message vocal reçu (file_id={file_id[:20]}…)"
                            )
                            ...
                            if audio_bytes:
                                ...
                                if transcribed:
                                    print(
                                        f"[ExternalBridge] Transcription: {transcribed[:80]}"
                                    )
                                    text = transcribed
                        if not text:
                            continue
                    ...
                    _bg_task(
                        handle_external_message("telegram", chat_id, text),
                        f"tg_msg_{chat_id}",
                    )
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    for m in messages:
                        ...
                        if text and sender:
                            print(
                                f"[ExternalBridge] WhatsApp message reçu: {text[:80]}"
                            )
                            _bg_task(
                                handle_external_message("whatsapp", sender, text),
                                f"wa_msg_{sender[:12]}",
                            )
            except asyncio.CancelledError:
                return
...
def start_bridge() -> list[asyncio.Task]:
    ...
    if _ada_loop is None:
        try:
            ...
            print(
                "[ExternalBridge] AudioLoop créée en mode standalone — capacités complètes actives."
            )
        except Exception as e:
            warnings.warn(
                f"[ExternalBridge] Impossible de créer AudioLoop standalone: {e} — fallback TextAgent."
            )
    tasks = [
