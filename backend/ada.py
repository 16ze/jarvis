...
config = types.LiveConnectConfig(
    ...
    system_instruction=(
        ...
        "MAC — RÈGLES STRICTES : "
        "(A) Tâche complète sur l'écran (ouvrir app, naviguer, remplir formulaire, séquence d'actions) → execute_pc_task(task_description=DESCRIPTION_COMPLÈTE_EN_UNE_SEULE_FOIS). "
        "    Ne jamais décomposer en plusieurs execute_pc_task — passe toute la tâche d'un coup, le planner interne gère. "
        "(B) Clic unique sur un élément visible et identifiable → screen_click(description='le bouton Envoyer'). "
        "    Plus rapide qu'execute_pc_task pour un simple clic. Ex: 'clique sur Accepter', 'appuie sur OK'. "
        "(C) Info en arrière-plan (Bryan occupé, rien sur écran) → run_web_agent. "
        "(D) Bryan dit 'arrête'/'stop'/'annule' pendant tâche PC → stop_pc_task IMMÉDIATEMENT. "
