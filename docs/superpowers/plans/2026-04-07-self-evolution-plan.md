# Ada Self-Evolution Agent — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Refactorer le system prompt d'Ada pour des appels d'outils immédiats, et créer un agent d'auto-évolution capable de générer, valider et déployer de nouveaux connecteurs MCP à chaud.

**Architecture:** Le `SelfEvolutionAgent` suit le même pattern que `SelfCorrectionAgent` (asyncio pur, Gemini 2.5 Flash, git backup). Il orchestre 6 étapes séquentielles : analyze → research → generate → validate → write → restart. Le redémarrage se fait via un script shell détaché, sans interaction humaine.

**Tech Stack:** Python 3.11, asyncio, google-genai, httpx, subprocess, tinytuya (existant)
...
|---|---|---|
| `backend/self_evolution_agent.py` | Créer | Agent complet : 6 étapes + injection de code |
| `backend/restart_ada.sh` | Créer | Script de redémarrage détaché |
| `backend/ada.py` | Modifier | System prompt refactor + init + wiring self_evolve |
| `backend/mcp_tools_declarations.py` | Modifier | Déclaration `self_evolve_tool` + ajout dans MCP_TOOLS |
| `backend/external_bridge.py` | Modifier | Wiring `self_evolve` dans `_execute_tool` |

---

## Task 1 : System Prompt Refactor

**Files:**
- Modify: `backend/ada.py:476-576`

- [ ] **Step 1 : Remplacer le system_instruction dans `config`**

Dans `ada.py`, remplacer tout le bloc `system_instruction=(...)` (lignes 476–576) par :
...
conda run -n ada_v2 python -m py_compile ada.py && echo "OK"
```
Résultat attendu : `OK`
...
git commit -m "refactor: system prompt ada — allégé 60%, appels tools immédiats"
```

---

## Task 2 : restart_ada.sh

**Files:**
- Create: `backend/restart_ada.sh`

- [ ] **Step 1 : Créer le script**

```bash
cat > /Users/bryandev/jarvis/backend/restart_ada.sh << 'EOF'
#!/bin/bash
...
EOF
chmod +x /Users/bryandev/jarvis/backend/restart_ada.sh
```

- [ ] **Step 2 : Tester le script en isolation**

```bash
bash /Users/bryandev/jarvis/backend/restart_ada.sh &
sleep 5
pgrep -f "python server.py" && echo "Serveur redémarré OK" || echo "ÉCHEC redémarrage"
```

- [ ] **Step 3 : Commit**

```bash
git add backend/restart_ada.sh
git commit -m "feat: restart_ada.sh — redémarrage auto post self-evolution"
```

---

## Task 3 : SelfEvolutionAgent — Infrastructure

**Files:**
- Create: `backend/self_evolution_agent.py`

- [ ] **Step 1 : Créer le fichier avec la classe de base et les helpers**
...
```bash
cd /Users/bryandev/jarvis/backend
conda run -n ada_v2 python -m py_compile self_evolution_agent.py && echo "OK"
```

---

## Task 4 : _analyze() — Identifier le service cible

**Files:**
- Modify: `backend/self_evolution_agent.py`

- [ ] **Step 1 : Ajouter la méthode `_analyze`**

Ajouter dans la classe `SelfEvolutionAgent`, après `_notify_telegram` :
...
result = asyncio.run(agent._analyze(
    goal='envoyer un SMS via Twilio',
    failed_context='outil twilio_send_sms non disponible'
))
print(result)
"
```
Résultat attendu : dict avec `service_name='twilio'`, `python_lib='twilio'`, etc.

---

## Task 5 : _research() — Scraper la documentation

**Files:**
- Modify: `backend/self_evolution_agent.py`

- [ ] **Step 1 : Ajouter la méthode `_research`**

```python
    async def _research(self, analysis: dict) -> str:
        """
        Fetche les URLs de documentation et retourne un extrait texte concaténé.
        Max 6000 tokens (~24000 chars) pour rester dans le contexte Gemini.
        """
        MAX_CHARS = 8000
        parts = []

        # Toujours fetcher PyPI en premier
...
from self_evolution_agent import SelfEvolutionAgent
agent = SelfEvolutionAgent()
analysis = {'python_lib': 'httpx', 'doc_urls': []}
doc = asyncio.run(agent._research(analysis))
print(doc[:500])
print('...', len(doc), 'chars')
"
```
Résultat attendu : texte avec description du package httpx, > 100 chars.

---

## Task 6 : _generate() — Générer le code MCP

**Files:**
- Modify: `backend/self_evolution_agent.py`

- [ ] **Step 1 : Lire les templates au démarrage**

Ajouter dans `__init__` après `self._client = ...` :
...
- [ ] **Step 2 : Ajouter la méthode `_generate`**
...
## Task 7 : _validate() — Import test avec retry

**Files:**
- Modify: `backend/self_evolution_agent.py`

- [ ] **Step 1 : Ajouter la méthode `_validate`**
...
ok, err = asyncio.run(agent._validate(
    'import os\nclass TestMCP:\n    def get(self) -> str:\n        return str(os.getcwd())\n',
    'test'
))
print('Valide:', ok, err)
# Test avec code invalide
ok2, err2 = asyncio.run(agent._validate('def broken(:\n    pass', 'test'))
print('Invalide:', ok2, err2)
"
```
Résultat attendu : `Valide: True ''` puis `Invalide: False 'SyntaxError: ...'`

---

## Task 8 : _write_files() — Injection dans les fichiers existants

**Files:**
- Modify: `backend/self_evolution_agent.py`

- [ ] **Step 1 : Ajouter la méthode `_write_files`**
...
```python
    def _write_files(self, analysis: dict, blocks: dict) -> str:
        ...
        try:
            ...
            # Chercher le marqueur MCP_TOOLS et insérer les déclarations avant
            insert_before = "\nMCP_TOOLS = ["
        ...
        try:
            ...
            if insert_before in content:
                ...
                # Assurer l'indentation correcte (16 espaces dans le bloc elif)
                dispatch_indented = "\n".join(
        ...
        try:
            ...
            insert_before = "        self._init_done = True" 
            # Chercher la dernière occurrence de try/except dans _init_agents
...
## Task 9 : _restart() — Redémarrage détaché

**Files:**
- Modify: `backend/self_evolution_agent.py`

- [ ] **Step 1 : Ajouter la méthode `_restart`**
...
## Task 10 : evolve() — Orchestrateur principal

**Files:**
- Modify: `backend/self_evolution_agent.py`

- [ ] **Step 1 : Ajouter la méthode `evolve`**
...
- [ ] **Step 2 : Vérifier syntaxe finale**

```bash
cd /Users/bryandev/jarvis/backend
conda run -n ada_v2 python -m py_compile self_evolution_agent.py && echo "OK"
conda run -n ada_v2 python -c "from self_evolution_agent import SelfEvolutionAgent; a=SelfEvolutionAgent(); print('Init OK')"
```
Résultat attendu : `OK` puis `Init OK`

- [ ] **Step 3 : Commit**

```bash
git add backend/self_evolution_agent.py
git commit -m "feat: self_evolution_agent.py — analyze/research/generate/validate/write/restart"
```

---

## Task 11 : Déclaration self_evolve_tool
...
Dans `mcp_tools_declarations.py`, trouver la section self-correction (vers la fin du fichier, avant `MCP_TOOLS = [`) et ajouter :
...
Dans la liste `MCP_TOOLS`, à la fin (avant le `]`), ajouter :
...
```bash
conda run -n ada_v2 python -c "from mcp_tools_declarations import MCP_TOOLS, MCP_TOOL_NAMES; print('self_evolve' in MCP_TOOL_NAMES)"
```
...
git commit -m "feat: self_evolve_tool déclaration Gemini"
```

---

## Task 12 : Wiring dans ada.py

**Files:**
- Modify: `backend/ada.py:690-723`
- Modify: `backend/ada.py:_execute_text_tool`

- [ ] **Step 1 : Importer et initialiser SelfEvolutionAgent dans `__init__`**

Dans `AudioLoop.__init__`, après le bloc `self_correction` (ligne ~696), ajouter :
...
- [ ] **Step 2 : Wirer dans `_execute_text_tool`**

Dans `_execute_text_tool`, avant le bloc `elif name == "refresh_tuya_devices":`, ajouter :
...
```bash
git add backend/ada.py
git commit -m "feat: wiring self_evolve dans ada.py + init SelfEvolutionAgent"
```

---

## Task 13 : Wiring dans external_bridge.py

**Files:**
- Modify: `backend/external_bridge.py`

- [ ] **Step 1 : Init dans `_init_agents`**

Dans `_init_agents`, après le bloc `MonitoringAgent` (dernier bloc try/except), ajouter :
...
Ajouter dans `__init__` : `self._evolution = None`

- [ ] **Step 2 : Dispatch dans `_execute_tool`**

Dans `_execute_tool`, avant `# ── DOMOTIQUE`, ajouter :
...
```bash
conda run -n ada_v2 python -m py_compile external_bridge.py && echo "OK"
```
...
git commit -m "feat: wiring self_evolve dans external_bridge.py"
```
...
```bash
bash /Users/bryandev/jarvis/start_ada.sh
```

- [ ] **Step 2 : Vérifier les logs de démarrage**

Dans les logs, vérifier :
```
[SERVER] Startup: Initializing Tuya Agent...
```
Et l'absence de `SelfEvolutionAgent init:` dans les warnings.

- [ ] **Step 3 : Test depuis Telegram**

Envoyer : `"teste self_evolve : j'ai besoin d'envoyer un SMS mais l'outil twilio_send_sms n'existe pas"`

Résultat attendu : Ada répond avec "Outil 'twilio' créé..." et le serveur redémarre dans 3s.

- [ ] **Step 4 : Vérifier les fichiers générés**

```bash
ls /Users/bryandev/jarvis/backend/mcps/ | grep twilio
git log --oneline -3
```
Résultat attendu : `twilio_mcp.py` présent, 2 commits récents (backup + feat).
...
**Spec coverage :**
- ✅ System prompt refactor (Task 1)
- ✅ restart_ada.sh détaché (Task 2)
- ✅ analyze → research → generate → validate × 3 → write → restart (Tasks 3–10)
- ✅ Escalade Telegram si 3 échecs (Task 10 `evolve()`)
- ✅ git backup avant écriture (Task 8)
- ✅ self_evolve_tool déclaré (Task 11)
- ✅ Wiring ada.py + external_bridge.py (Tasks 12–13)

**Placeholders :** Aucun — tout le code est écrit.

**Cohérence types :** `evolve(goal: str, failed_context: str) -> str` utilisé de façon cohérente dans Tasks 10, 12, 13. `_validate_with_retry` retourne `Optional[dict]` consommé dans Task 10.
