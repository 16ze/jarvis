...
const CadWindow = ({ data, thoughts, retryInfo = {}, onClose, socket }) => {
    ...
    return (
        <div className="w-full h-full relative group bg-slate-900 rounded-lg overflow-hidden border border-blue-400/40">
            {/* Close Button */}
            ...
            <div className="absolute top-2 left-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                <button
                    ...
                    className="bg-blue-500/20 hover:bg-blue-500/40 text-blue-700 text-xs px-2 py-1 rounded border border-blue-400/40 backdrop-blur-sm"
                >
                    ITERATE
                </button>
                <button
            ...
            {(isIterating || (!data && data?.format !== 'loading')) && (
                <div className={`absolute inset-0 z-20 ${!data ? 'bg-white/70' : 'bg-white/60'} backdrop-blur-md flex items-center justify-center p-4`}>
                    <div className="bg-white/80 border border-blue-400/40 rounded p-4 w-full max-w-sm pointer-events-auto shadow-[0_0_20px_rgba(59,130,246,0.2)]">
                        <h4 className="text-blue-700 text-sm mb-2 font-mono">
                        ...
                        <textarea
                            ...
                            className="w-full bg-white/70 border border-blue-200 rounded p-2 text-slate-800 text-sm mb-3 focus:outline-none focus:border-blue-500 h-24 resize-none placeholder:text-slate-400"
                            autoFocus
                            onKeyDown={(e) => {
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            {data && (
                                <button
                                    ...
                                    className="text-slate-500 text-xs hover:text-slate-800 px-2 py-1"
                                >
                                    Cancel
                                </button>
                            )}
            ...
            {data?.format === 'loading' && (
                <div className="absolute inset-y-0 right-0 w-2/5 p-4 bg-white/70 backdrop-blur-sm border-l border-green-500/30 overflow-hidden flex flex-col">
                    <div className="flex items-center justify-between mb-2">
                        <h4 className="text-green-600 text-xs font-mono tracking-widest uppercase flex items-center gap-2">
                            <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
                            Designer Thinking...
                        </h4>
                        {retryInfo.attempt && (
                            <span className={`text-xs font-mono px-2 py-0.5 rounded ${retryInfo.error ? 'bg-yellow-500/20 text-yellow-400' : 'bg-blue-500/20 text-blue-700'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto text-green-700/90 text-xs font-mono whitespace-pre-wrap leading-relaxed scrollbar-thin scrollbar-thumb-green-500/30">
                        {thoughts}
            ...
            <div className="absolute bottom-2 left-2 text-[10px] text-blue-300/70 font-mono tracking-widest pointer-events-none">
